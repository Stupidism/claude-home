...
**Agent `mcp__github__*` writes return `403 Resource not accessible by personal access token`**
→ The agent's GitHub MCP server (configured under `mcpServers.github` in `~/.claude.json`, user-global, not version-controlled) must use a **classic** PAT with scopes `repo`, `project`, `write:discussion`. Fine-grained PATs structurally cannot write to cross-account user-owned repos/projects, so MCP writes (`add_issue_comment`, `issue_write`) 403 even when the scope looks correct. The **same classic-PAT requirement** applies to `secrets.env`'s `GITHUB_TOKEN` — used by the poller's github-projects adapter and the skill's GraphQL writes (not the MCP server): a fine-grained PAT there will 403 on cross-account user-owned ProjectV2 mutations for the same reason.
