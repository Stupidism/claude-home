...
```
# Jira: try email first (most reliable), then accountid: prefix
...
Then read the ticket back and confirm:
...
- **Jira `assignee` dropped on create** — confirmed empirically; must follow up with `jira_update_issue`. Email worked when `accountid:` prefix didn't.
...
- **Wrong assigneeId for the board** — `symphony.json` has a Linear UUID; `boards/up.json` has a Jira accountId. Don't cross them.
