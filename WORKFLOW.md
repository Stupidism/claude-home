...
| `In Progress`  | 从 workpad 恢复。只有当 ① 无新开发者指令 ② 无未解决 PR 评论 ③ CI 绿 ④ **AI code review 已对当前 PR head 跑完一轮**（工单上有 `[symphony] aiReviewRequested:` 锁评论且 PR 上有对应的 AI 回复，例如 Codex / CodeRabbit）且无遗留发现 — 才算"没有可执行的工作"，此时必须调用 `submit-for-review` 把工单推回 `Human Review`，不得静默退出。若 AI 评审未完成或有发现，按反馈处理而非直接退出。 |
