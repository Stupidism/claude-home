...
>
> **工单系统：** 通过 `$SKILLS_ROOT/ticket/SKILL.md` 调度器路由 — Linear (`WOR-*`) 走 `linear/SKILL.md`、Jira (`UP-*`) 走 `jira/SKILL.md`。两边都优先使用对应的 MCP 工具（`mcp__linear-server__*` 或 `mcp__claude_ai_Atlassian__*`），仅当 MCP 工具在当前会话不可用时才回退到 curl。**如果获取工单失败，立即停止并报告错误 — 不得根据标题或代码库猜测需求。**
>
